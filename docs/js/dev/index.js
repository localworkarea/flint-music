...
addLoadedAttr();
var plativka = document.querySelector(".plativka");
window.addEventListener("load", () => {
	setTimeout(() => {
		if (plativka) plativka.classList.add("--continue-animation");
	}, 2500);
});
document.addEventListener("DOMContentLoaded", () => {
	...
	const lyricsData = [
		{
			...
			text: "Flint Flint Flint"
		},
		{
			start: 3.51,
			end: 5.5,
			text: "Всі ми любим грінки"
		},
		{
			start: 5.51,
			end: 7,
			text: "Грінки грінки Flint"
		},
		{
			start: 7.01,
			end: 9.5,
			text: "З гарячими смаками"
		},
		{
			start: 9.51,
			end: 11,
			text: "Запашний часник"
		},
		{
			start: 11.01,
			end: 13,
			text: "Пікантні гострі джерки"
		},
		{
			start: 13.01,
			end: 15,
			text: "Flintовські гострі джерки"
		},
		{
			start: 15.01,
			end: 16.5,
			text: "Спайсі та томат"
		},
		{
			start: 16.51,
			end: 19,
			text: "Flint Спайсі та томат"
		},
		{
			start: 19.01,
			end: 21,
			text: "Хвилястий житній хлібчик"
		},
		{
			start: 21.01,
			end: 23,
			text: "Брусочком житній хлібчик"
		},
		{
			start: 23.01,
			end: 27.5,
			text: "Flint грінки грінки, ковтаю слинки"
		},
		{
			start: 27.51,
			end: 30,
			text: "FLINTКРИВАЙ крафтові смаки"
		}
	];
	const framesData = [
		{
	...
	function bindEvents() {
		...
		audio.addEventListener("pause", () => {
			if (!audio.ended) flintPlayer?.classList.add("--pause");
			updateButtonsState(false);
			stopRAF();
			pauseDiscRotation();
		});
	...
	function togglePlayback() {
		if (!plativka.classList.contains("--active")) plativka?.classList.add("--active");
		if (!flintPlayer.classList.contains("--active")) flintPlayer?.classList.add("--active");
		if (!framePack.classList.contains("--active")) framePack?.classList.add("--active");
		if (audio.paused) if (!firstPlayDelayUsed) {
	...
	function updateTimeUI(current, duration) {
		const remaining = Math.max((Number.isFinite(duration) ? duration : 0) - current, 0);
	...
	function updateDiscRotation(now) {
		const delta = (now - discLastTime) / 1e3;
		discLastTime = now;
		discSpeed += (discTargetSpeed - discSpeed) * Math.min(delta * 1.8, 1);
		discAngle += discSpeed * delta;
		if (plativkaDisc) plativkaDisc.style.transform = `rotate(${discAngle}deg)`;
		if (Math.abs(discSpeed) > .01 || discTargetSpeed !== 0) discRafId = requestAnimationFrame(updateDiscRotation);
		else {
			discSpeed = 0;
			discRafId = null;
		}
...
//#endregion
//#region src/components/custom/video/video.js
document.querySelectorAll(".video-youtube__button").forEach((button) => {
	button.addEventListener("click", function() {
		let urlVideo = `https://www.youtube.com/embed/${this.getAttribute("data-youtube")}?rel=0&showinfo=0`;
...
function initSliders() {
	if (document.querySelector(".flint__slider")) new Swiper(".flint__slider", {
		modules: [Navigation],
		observer: true,
		observeParents: true,
		slidesPerView: 1,
...
document.querySelector("[data-att-slider]") && window.addEventListener("load", initSliders);
//#endregion
...
var FullPage = class {
	constructor(element, options) {
		let config = {
			...
			idActiveSection: 0,
			mode: element.dataset.effect ? element.dataset.effect : "slider",
	...
	setId() {
		for (let index = 0; index < this.sections.length; index++) this.sections[index].setAttribute("data-fp-id", index);
	}
	removeId() {
		for (let index = 0; index < this.sections.length; index++) this.sections[index].removeAttribute("data-fp-id");
	}
	setClasses() {
		...
		for (let index = 0; index < this.sections.length; index++) document.documentElement.classList.remove(`--section-${index}`);
...
if (document.querySelector("[data-att-fullpage]")) window.addEventListener("load", () => window.flsFullpage = new FullPage(document.querySelector("[data-att-fullpage]")));
//#endregion
